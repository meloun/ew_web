    ...
    return this.each(function(){
      ...
      $.getJSON(url, function(data){
        ...
        $.each(data.results, function(i,item){
          ...
          var date = '<div class="date">'+relative_time(item.created_at)+'</div>';
          var text = '<span class="tweet_text">' +$([item.text]).linkUrl().linkUser().linkHash().makeHeart().capAwesome().capEpic()[0]+ '</span>';
          
          // until we create a template option, arrange the items below to alter a tweet's display.
          //list.append('<li>' + avatar + date + join + text + '</li>');
	  list.append('<li>'+ join + text + date +'</li>');


        });
	list.append('<li><a href=\"http://twitter.com/esportuj\"><img src=\"http://www.twitterbuttons.com/images/lbn/twitterbutton-0208.png\" title=\"Follow us on Twitter\" width=\"120\" height=\"90\" /></a></li>');
	list.children('li:first').addClass('tweet_first');
        list.children('li:odd').addClass('tweet_even');
        list.children('li:even').addClass('tweet_odd');
	
        if (s.outro_text) list.after(outro);
	//$("<h1>dd</h1>").appendTo(intro.before("intro"));
      });
