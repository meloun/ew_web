...
class User {
	...
	public static function Add(&$array, $replace_permission = 0) {	    
	    ...
	    $kategory_id_esc = Db::EscInteger($array['kategory_id']);
            $club_esc = Db::EscString($array['club']);
	    ...
	    $query = "
		    REPLACE INTO users
		    (id, competition_id, kategory_id, first_name, last_name, club, email, year, sex, user_field_1, user_field_2, user_field_3,user_field_4, symbol)
		    VALUES
		    ({$id_esc}, {$competition_id_esc}, {$kategory_id_esc}, {$first_name_esc}, {$last_name_esc}, {$club_esc}, {$email_esc}, {$year_esc}, {$sex_esc}, {$user_field_1_esc}, {$user_field_2_esc}, {$user_field_3_esc}, {$user_field_4_esc}, {$symbol_esc})
	    ";
            var_dump($query);
	    Db::Query($query, LINK);
